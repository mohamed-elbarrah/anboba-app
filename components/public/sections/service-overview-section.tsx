    ...
    >
      <div className="mx-auto max-w-[1440px]">
        <div className="grid min-h-[273px] items-center gap-8 overflow-hidden rounded-[3.5rem] [direction:ltr] border border-white/90 bg-white/50 px-6 py-8 shadow-[0_5px_8px_rgb(111_78_58_/_10%)] sm:gap-10 sm:px-10 sm:py-9 lg:grid-cols-[180px_minmax(0,1fr)] lg:gap-16 lg:px-[5.5rem] lg:py-10">
          <MotionReveal className="flex justify-center [direction:ltr] lg:justify-start" x={-35}>
            {isExternalSource(imageSrc) ? (
              /* eslint-disable-next-line @next/next/no-img-element */
              <img src={imageSrc} alt={content.imageAlt} width={ANBOBA_IMAGE_DIMENSIONS.width} height={ANBOBA_IMAGE_DIMENSIONS.height} className="h-auto w-[min(52vw,180px)] object-contain sm:w-[180px] lg:w-[180px]" />
            ) : (
              <Image src={imageSrc} alt={content.imageAlt} width={ANBOBA_IMAGE_DIMENSIONS.width} height={ANBOBA_IMAGE_DIMENSIONS.height} sizes="(max-width: 639px) 180px, (max-width: 1023px) 180px, 180px" className="h-auto w-[min(52vw,180px)] object-contain sm:w-[180px] lg:w-[180px]" />
            )}
          </MotionReveal>
          <MotionReveal className="text-center lg:text-start" x={35} delay={0.2}>
