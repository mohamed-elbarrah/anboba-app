...
export default async function SiteHeader({ locale }: { locale: Locale }) {
  ...
  return (
    <header className="px-4 pt-5 sm:px-6 sm:pt-7" dir={locale === "ar" ? "rtl" : "ltr"}>
      <div className="mx-auto flex min-h-16 max-w-[800px] items-center justify-between gap-4 rounded-full border border-border/80 bg-background/95 px-4 py-2 shadow-[0_12px_36px_color-mix(in_srgb,var(--foreground)_8%,transparent)] backdrop-blur sm:px-6">
        <Link href={`/${locale}`} aria-label={dictionary.pages.logoLabel} className="shrink-0 rounded-md focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary">
