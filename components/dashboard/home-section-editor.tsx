          ...
          <div className="grid gap-3 md:grid-cols-2">
            {fields.map((field) => (
              <TextField
                key={field}
                label={field}
                value={text(items[index] ?? {}, field)}
                multiline={field.toLowerCase().includes("description")}
                onChange={(value) => {
                  const next = [...items];
...
function GuaranteeItems({
  ...
  return (
    <div className="space-y-3">
      {[0, 1].map((offset) => (
        <div key={offset} className="rounded-lg border bg-muted/20 p-4">
          <p className="mb-3 text-sm font-semibold">
            Card {start + offset + 1}
          </p>
          <div className="space-y-3">
            <TextField
              label="Card title (shared by all cards)"
              value={label}
              onChange={onLabelChange}
            />
            <TextField
              label={`Card ${start + offset + 1} paragraph`}
              value={String(values[start + offset] ?? "")}
              onChange={(value) => {
                const next = [...values];
                next[start + offset] = value;
                onChange(next);
              }}
            />
          </div>
...
function NestedGroup({
  ...
  return (
    <div className="rounded-lg border bg-muted/20 p-4">
      ...
      <Fields
        data={nested}
        names={["heading", "description", "cta", "href"]}
...
export function HomeSectionEditor({
  ...
  switch (section.key) {
    case "hero": {
      ...
      return (
        <div className="space-y-4">
          <Group
            title="Hero content"
            description="Edit the text in the same order it appears above the showcase."
          >
            <Fields
              data={data}
              names={[
          ...
          <Group
            title="Showcase"
            description="The public layout has one shared card title and two paragraphs on each side."
          >
            <div className="grid gap-4 md:grid-cols-2">
              <TextField
                ...
                onChange={(value) =>
                  setShowcase({ ...showcase, heading: value })
                }
              />
              <TextField
                label="Card title (shared by all cards)"
                value={text(showcase, "guaranteeLabel")}
                onChange={(value) =>
                  setShowcase({ ...showcase, guaranteeLabel: value })
                }
              />
            </div>
          </Group>
            ...
            {[
              ...
              <Card key={slot.id}>
                <CardHeader>
                  ...
                  <CardDescription>
                    Each side contains two public guarantee cards and one phone
                    mockup.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <GuaranteeItems
                    values={guarantees}
                    start={slot.start}
                    label={text(showcase, "guaranteeLabel")}
                    onLabelChange={(value) =>
                      setShowcase({ ...showcase, guaranteeLabel: value })
                    }
                    onChange={(next) =>
    ...
    case "statistics":
      return (
        <Group title="Statistics">
          <Fields data={data} names={["heading"]} update={set} />
          <Items
            data={data}
            name="items"
            count={4}
      ...
      return (
        <Group title="Why choose us">
          <Fields
            ...
            update={set}
            multiline={["subtitle", "cardParagraph"]}
          />
          <StringItems data={data} name="features" count={4} update={set} />
        </Group>
      );
    case "service_benefits":
      return (
        <Group title="Service benefits">
          <Fields
            data={data}
            names={["eyebrow", "headingHighlight", "headingRest", "subtitle"]}
            update={set}
            multiline={["subtitle"]}
          />
          <Items
    ...
    case "join_application":
      return (
        <Group title="Join application">
          <Fields
            data={data}
            names={["heading", "description", "fileHint", "note"]}
            update={set}
            multiline={["description", "note"]}
          />
          <StringItems data={data} name="benefits" count={3} update={set} />
        </Group>
      );
    case "faq_support":
      return (
        <Group title="FAQ and support">
          <NestedGroup data={data} name="faq" update={set} />
          <NestedGroup data={data} name="support" update={set} />
        </Group>
      );
    default:
      return null;
