    ...
    <Card>
      <CardContent className="space-y-3 p-4">
        ...
        <dialog ref={dialogRef} aria-labelledby={titleId} className="m-auto max-h-[min(90vh,720px)] w-[min(calc(100%-2rem),680px)] rounded-xl border bg-background p-0 text-foreground shadow-xl backdrop:bg-black/50">
          <div className="flex items-center justify-between border-b p-4"><h2 id={titleId} className="text-lg font-semibold">Select an image</h2><Button type="button" variant="ghost" size="sm" onClick={closeDialog} aria-label={ui.close}><X /></Button></div>
          <div className="space-y-5 overflow-y-auto p-4">
            ...
            <div className="space-y-2 border-t pt-4"><label htmlFor={urlId} className="text-sm font-medium">{ui.external}</label><div className="flex gap-2"><Input id={urlId} type="url" inputMode="url" placeholder={ui.placeholder} value={url} onChange={(event) => setUrl(event.target.value)} /><Button type="button" onClick={selectExternal}>{ui.useUrl}</Button></div><p className="text-xs text-muted-foreground">{ui.hint}</p></div>
          </div>
        </dialog>
      </CardContent>
    </Card>
