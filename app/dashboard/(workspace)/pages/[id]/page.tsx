...
export default async function PageEditorRoute({ params }: Props) {
  ...
  try {
    ...
    return (
      <EditorError
        message="The editor could not be loaded. Check the content service configuration."
  ...
  if (!requested)
    return <EditorError message="The requested page was not found." />;
  ...
  const documents: {
    ar: Awaited<ReturnType<typeof getEditorDocument>>;
    en: Awaited<ReturnType<typeof getEditorDocument>>;
  } = { ar: null, en: null };
  const errors: { ar: string | null; en: string | null } = {
    ar: null,
    en: null,
  };
  await Promise.all(
    (["ar", "en"] as const).map(async (locale) => {
      ...
      try {
        ...
        console.error(
          `[dashboard/pages/${id}] Unable to load ${locale} editor`,
          error,
        );
  ...
  const media =
    requested.slug === "" || requested.slug === "about"
      ? await listMedia()
      : [];
  ...
  return (
    <PageEditor
      ...
      isHome={requested.slug === ""}
      visualPage={requested.slug}
      media={media}
      forms={forms}
    />
  );
    ...
    <main className="mx-auto w-full max-w-3xl p-4 md:p-8">
      <div className="rounded-lg border border-destructive/30 bg-card p-10 text-center">
        ...
        {process.env.NODE_ENV === "development" && detail && (
          <p className="mt-2 break-all text-xs text-destructive">{detail}</p>
        )}
      </div>
    </main>
  );
