...
const authRequired = {
  ok: false,
  code: "AUTH_REQUIRED",
  message: "Authentication is required for page mutations",
} as const;
...
const failures = {
  ...
  notFound: {
    ok: false,
    code: "NOT_FOUND",
    message: "Page or draft was not found",
  } as const,
  stale: {
    ok: false,
    code: "STALE_REVISION",
    message: "The draft is out of date",
  } as const,
  invalidDocument: {
    ok: false,
...
type ValidDocument = z.infer<typeof documentSchema> & {
  locale: "ar" | "en";
  pageId: string;
...
const builtInFormForSection = {
  contact: "contact",
  join_application: "join_application",
...
async function validateSections(
  ...
  for (const [index, section] of document.sections.entries()) {
    ...
    try {
      if (
        key === "contact" ||
        key === "join_application" ||
        key === "partner_registration"
      )
        parseStrictSectionOwnedFormContent(key, section.content);
      else parseSectionContent(key, section.content);
    } catch {
      return false;
    }
    const expectedForm =
      builtInFormForSection[key as keyof typeof builtInFormForSection];
    if (key === "join_application") {
      ...
      const form = (
        await tx
          .select()
          .from(forms)
          .where(eq(forms.id, BigInt(section.formId)))
          .limit(1)
      )[0];
      if (
        !form ||
        form.archived ||
        (form.kind === "system" && form.rendererKey !== "join_application") ||
        (form.kind === "user" && form.rendererKey !== "generic")
      )
        return false;
      ...
      if (
        ...
        !(
          (form.kind === "system" &&
            form.formKey === expectedForm &&
            form.rendererKey === expectedForm) ||
          (form.kind === "user" && form.rendererKey === "generic")
        )
      )
        return false;
...
async function loadTarget(
  ...
  const page = (
    await tx
      .select()
      .from(pages)
      .where(and(eq(pages.id, pageId), eq(pages.locale, document.locale)))
      .limit(1)
  )[0];
  if (!page) return null;
  ...
  const pointer = (
    await tx
      .select()
      .from(pageRevisionPointers)
      .where(eq(pageRevisionPointers.pageId, pageId))
      .limit(1)
      .for("update")
  )[0];
  if (!pointer?.draftRevisionId) return null;
  if (pointer.draftRevisionId.toString() !== document.revisionToken)
  ...
  const draft = (
    await tx
      ...
      .where(
        and(
          eq(pageRevisions.id, pointer.draftRevisionId),
...
async function runMutation(
  ...
  try {
    ...
    if (result.ok && result.revalidationSlug !== undefined)
      safeRevalidate(locale, result.revalidationSlug);
...
export async function saveDraft(
  ...
  return runMutation("save-draft", document.locale, async () => {
    const result = await getDb().transaction(async (tx) => {
      ...
      if (!(await validateSections(tx, document, target.page.slug)))
        return invalidDocument();
      await tx
        .update(pageRevisions)
        .set({ status: "archived" })
        .where(eq(pageRevisions.id, target.draft.id));
      const revisionId = await insertRevision(
        tx,
        document,
        target.page.id,
        "draft",
        await nextRevisionNumber(tx, target.page.id),
      );
      await tx
        .update(pageRevisionPointers)
        .set({ draftRevisionId: revisionId })
        .where(eq(pageRevisionPointers.pageId, target.page.id));
      return {
        ok: true,
        revisionToken: revisionId.toString(),
        status: "draft",
        revalidationSlug: target.page.slug,
      } as const;
    });
    return result;
  });
...
export async function publishPage(
  ...
  return runMutation("publish-page", document.locale, async () => {
    return getDb().transaction(async (tx) => {
      ...
      if (!(await validateSections(tx, document, target.page.slug)))
        return invalidDocument();
      if (target.pointer.publishedRevisionId)
        await tx
          .update(pageRevisions)
          .set({ status: "archived" })
          .where(eq(pageRevisions.id, target.pointer.publishedRevisionId));
      await tx
        .update(pageRevisions)
        .set({ status: "archived" })
        .where(eq(pageRevisions.id, target.draft.id));
      const publishedId = await insertRevision(
        tx,
        document,
        target.page.id,
        "published",
      ...
      const draftId = await insertRevision(
        tx,
        document,
        target.page.id,
        "draft",
      ...
      await tx
        .update(pageRevisionPointers)
        .set({ publishedRevisionId: publishedId, draftRevisionId: draftId })
        .where(eq(pageRevisionPointers.pageId, target.page.id));
      return {
        ok: true,
        revisionToken: draftId.toString(),
        status: "published",
        revalidationSlug: target.page.slug,
      } as const;
    });
...
export async function discardDraft(
  ...
  if (!(await getCurrentAdmin())) return authRequired;
  const parsed = z
    .object({ pageId: idSchema, locale: z.string(), revisionToken: idSchema })
    .safeParse({ pageId, locale, revisionToken });
  ...
  return runMutation("discard-draft", locale, async () => {
    return getDb().transaction(async (tx) => {
      ...
      if (target === "stale") return failures.stale;
      if (!target || !target.pointer.publishedRevisionId)
      ...
      const published = (
        await tx
          ...
          .where(
            and(
              eq(pageRevisions.id, target.pointer.publishedRevisionId),
              eq(pageRevisions.pageId, target.page.id),
      ...
      const sections = await tx
        .select()
        .from(pageSections)
        .where(eq(pageSections.revisionId, published.id))
      ...
      const document: ValidDocument = {
        ...
        sections: sections.map((section) => ({
          ...
          sortOrder: section.sortOrder,
          formId: section.formId === null ? null : section.formId.toString(),
          content:
            section.formId !== null &&
            (section.sectionKey === "contact" ||
              section.sectionKey === "join_application" ||
              section.sectionKey === "partner_registration")
              ? parseSectionOwnedFormContent(
                  section.sectionKey,
                  section.contentJson,
                )
